...
func NewDashboardService(cache *cache.Cache, store *repository.Store) *DashboardService {
	return &DashboardService{cache: cache, store: store}
}

func (s *DashboardService) Summary(ctx context.Context, userID, month string) (*model.DashboardSummary, error) {
	if month == "" {
	...
	summary, err := s.store.GetTransactionSummary(ctx, userID, month)
	if err != nil {
		return nil, err
	}
...
func (s *DashboardService) Chart(ctx context.Context, userID, month string) (*model.ChartData, error) {
	if month == "" {
		month = time.Now().Format("2006-01")
	}
	...
	chart, err := s.store.GetTransactionChartData(ctx, userID, month, user.Timezone)
	if err != nil {
		return nil, err
	}
...
func (s *DashboardService) Report(ctx context.Context, userID, period, date string) (*model.PeriodReport, error) {
	if period == "" {
		period = "monthly"
	}
	if date == "" {
